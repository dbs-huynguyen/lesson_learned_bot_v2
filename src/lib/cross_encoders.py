...
class HuggingFaceCrossEncoder(BaseModel):
    ...
    def score(self, text_pairs: List[Tuple[str, str]], batch_size: int = 4) -> List[float]:
        ...
        Args:
            ...
            batch_size (int, optional): The batch size to use when scoring the text pairs. Defaults to 4.

        Returns:
            List[float]: List of scores, one for each pair.
        """
        scores = self.client.predict(text_pairs, batch_size=batch_size)
