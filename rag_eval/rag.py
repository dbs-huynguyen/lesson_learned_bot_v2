...
class RAGAgent:
    ...
    async def async_invoke(self, input: dict) -> dict:
        ...
        try:
            ...
            context = [
                Document(doc.get("page_content"), metadata=doc.get("metadata"))
                for doc in docs
            ]
    ...
    async def batch_invoke_async(
        self, inputs: t.List[dict], max_concurrent: int = 5
    ) -> t.List[dict]:
        """
        ...
        async def bounded_invoke(index: int, input_dict: dict, pbar: tqdm) -> None:
            ...
            async with semaphore:
                try:
                    ...
                    print(
                        f"Error processing question '{input_dict.get('question', '')}': {e}"
                    )
        ...
        with tqdm(total=len(inputs), desc="Processing RAG queries", leave=True) as pbar:
            tasks = [
                bounded_invoke(i, input_dict, pbar)
    ...
    def batch_invoke_with_executor(
        self, inputs: t.List[dict], max_workers: int = 5
    ) -> t.List[dict]:
        """
